...
```yaml
- uses: JosiahSiegel/umactually-action@9bfdeeb213811dd9e492bcf6a888d6fd319b84b1  # v1
```
...
| --- | --- | --- | --- |
| `cli-version` | no | `__UMACTUALLY_VERSION__` | `umactually` CLI version to install via `npm install -g`. The placeholder `__UMACTUALLY_VERSION__` is substituted by `umactually init` at wizard time; pin a specific tag (e.g. `0.12.0`) for hand-written files. |
...
| `effort` | no | `""` | Optional reasoning effort: `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, or `max` (requires a CLI version with `--effort` support, i.e. `0.12.0+`). Forwarded as `--effort` only when non-empty; empty defers to a caller-provided `UMACTUALLY_EFFORT` env var, then saved config, then the provider/model default. |
